//
//  Created by Nazar Alwi on 22/05/21
...
class EssentialFeedSecondCacheIntegrationTests: XCTestCase {
    override func setUp() {
        super.setUp()
    ...
    func test_saveFeed_overridesItemSavedOnASeparateInstance() {
    ...
    func test_loadImageData_deliversSavedDataOnASeparateInstance() {
        let imageLoaderPerformSave = makeImageLoader()
        let imageLoaderPerformLoad = makeImageLoader()
        let feedLoader = makeFeedLoader()
        let image = uniqueImage()
        let dataToSave = anyData()
        
        save([image], with: feedLoader)
        save(dataToSave, for: image.url, with: imageLoaderPerformSave)
        
        expect(imageLoaderPerformLoad, toLoad: dataToSave, for: image.url)
    }
